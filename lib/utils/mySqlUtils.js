...
/**
  * insert operation
  */
DBUtils.prototype.executeInsert = function(sql, params){
  if (params === undefined) params = [];
  var query = this.connection.query(sql, params, function(err, result) {
    if (err) {
      return handle_error(err);
    }
    logger.info("exec sql: "+ query.sql + ", generated pk =  " + result.insertId);
  });
  return query;
}
